...
def handle_on_success(job_type, result):
    ...
    Called by workers after mark_job_succeeded. Uses per-type dispatch
    for queue functions that have special pre-conditions.
    """
    ...
    for followup_type in on_success:
        if followup_type == 'plex_library_update':
            job_id = queue_plex_library_update(trigger=f'post_{job_type}')
            if job_id:
                logger.info("[ON_SUCCESS] %s → queued plex_library_update job %s", job_type, job_id)
        ...
        elif followup_type == 'plex_listen_history_sync':
